...
const Header: React.FC<HeaderProps> = ({ currentRoom, displayName, onRoomClick, onDisplayNameClick }) => {
  ...
  return (
    <header className="flex items-center justify-between p-4 bg-surface border-b border-border shadow-md">
      <div className="flex items-center space-x-2">
        <div className={`w-3 h-3 rounded-full ${isConnected ? 'bg-success animate-pulse' : 'bg-gray-500'}`}></div>
        <h1 className="text-lg font-bold text-text">Somnia On-Chain Chat</h1>
      </div>
        ...
        {isConnected && (
          <>
            {currentRoom && (
              <button
                onClick={onRoomClick}
                className="hidden sm:flex items-center space-x-2 px-3 py-1.5 bg-surface-light hover:bg-background rounded-md transition-colors text-sm"
                title="Switch room"
              >
